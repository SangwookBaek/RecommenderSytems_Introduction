...
class My_AssociationRecommender(BaseRecommender):
    def recommend(self, dataset: Dataset, **kwargs) -> RecommendResult:
        # 평갓값의 임곗값
        min_support = kwargs.get("min_support", 0.1)
        min_threshold = kwargs.get("min_threshold", 1)
        ...
        # 지지도가 높은 영화
        freq_movies = apriori(user_movie_matrix.astype(bool), min_support=min_support, use_colnames=True)
        # 어소시에이션 규칙 계산(리프트값이 높은 순으로 표시)
        rules = association_rules(freq_movies, metric="lift", min_threshold=min_threshold)
...
if __name__ == "__main__":
    #AssociationRecommender(data_loader_str='arrow').run_sample()
    My_AssociationRecommender(data_loader_str='arrow').run_sample()
